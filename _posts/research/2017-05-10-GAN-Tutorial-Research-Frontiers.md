...
[制作中, 最后修改时间： 2017年5月24日]

### 5.1.1 Mode collapse

Mode collapse， 也被称为Helvetica scenario， 此问题发生于当生成器试图映射几个不同的$$z$$输入到同一个输出点时。
在实际应用中， 完全的模式塌陷是很少的， 但是部分的Mode collapse是很常见的。 
部分的Mode collapse场景是， 当生成器将多个图像包含一样的颜色或者同一个本质的主题， 或者多个图像包含一个狗的不同的视角。 
图22展示了Mode collapse的问题。

![Figure 22](/images/201705/10/fig22.jpg)

图22： 在一个二维模拟数据上的Mode collapse问题演示。 最上面的是模型应该学习的目标分布$$p_{data}$$。
...
Mode collapse的发生的可能是因为GAN使用最大最小（maximin）优化与使用minimax不同。 当我们寻找模型使用
...
正如3.2.6章讨论的， Mode collapse不像是被任何特别的损失函数引起的。
...
因为Mode collapse问题， GAN的应用经常限定到那些模型产生比较少不同的输出的问题。
...
Mode collapse问题很可能是GAN最重要的问题需要研究者去解决。
...
另一个解决Mode collapse问题的方法是unrolled GAN （Metz et al., 2016)。 理想情况下， 我们希望找到$$G^{*}=arg min_G max_D V(G,D）$$ 对两个玩家，我们会忽略掉max操作当计算G的梯度时。
...
如果我们的关于如何理解是否一个连续，高维，非凸的游戏将收敛的理论可以被改善， 或者如果我们可以开发算法来让收敛更可靠比同步梯度下降， 除了GAN以外的好多领域都会受益。
即便是仅仅AI研究，我们也可以找到很多的应用场景：
...
另一个很重要的关于GAN的研究领域是目前还不清楚如何定量的去评价生成模型。
模型能得到好的似然，可能产生坏的样本， 并且模型产生好的样本一样也可能有很差的似然。
还没有明确的判定方式来定量的给样本打分。
GAN有些比其他生成模型难评价因为比较困难去估计GAN的似然（但是他是可能的 请参考Wu et al. (2016))。
...
去掉这个限制是一个很重要的研究方向， 将会解锁GAN应用与NLP的潜力。
这里有至少三个明显的方式可以处理此问题：
...
GAN以及被成功的应用到了半监督学习至少从CatGANs (Springenberg, 2015)。 当前， 最好的半监督学习结果在MNIST, SVHN, 和 CIFAR-10 是由feature matching GAN得到的 (Salimans et al., 2016)。
通常，模型使用50000或者更多的标定数据进行训练， 但是feature matching GAN可以得到很好的表现仅仅使用很少的训练数据。
在好几个使用不同量训练样本（从20到8000）的任务上， 它都得到了最好的性能表现。

如何使用使用feature matching GAN来做半监督学习的基本思想是是将一个$$n$$类的分类问题修改为一个$$n+1$$类的分类问题， 通过增加一个对应伪样本的辅助的类。
在GAN中使用分类器来作为判别器， 可以通过将所有的真实的类相加来得到真实样本的概率。 
Real/Fake的判定可以甚至使用非标定的真实数据来训练， 并且也可以使用被标定为Fake的由生成器生成的数据来训练。
分类器可以被训练来识别独立的真实的类在有限的真实，标定的样本上。 这个方法被Salimans et al. (2016) and Odena
(2016)同时提出来。
...
**GAN学习一个关于图像x的表达$$z$$。**
以及知道这个表达可以捕捉$$x$$的有用的，高级的，概要的，有语义的属性， 但是还有些困难来使用这些信息。

一个使用z的障碍是很难获得z通过给出一个$$x$$。 
Goodfellow et al. (2014b) 建议使用第二个网络模拟生成器来从$$p(z|x)$$采样， 很像生成器从$$p(x)$$采样， 但是没有给出实现。
尽管此思路的完整版本，也就是使用一个完全的通用的神经网络作为encoder并从模拟的$$p(z|x)$$的强劲的近似中采样, 没有被成功的实现，
但是Donahue et al. (2016)展示了如何训练一个确定性encoder， 并且Dumoulin et al. (2016) 展示了如何训练一个encoder网络， 
从高斯先验分布中采样。 进一步的演技将开发更强大的随机encoder。

另一种很好的使用code的方式是训练这个code并使其更有用。
InfoGANs (Chen et al., 2016a)通过来标准化code向量中的一些输入，通过使用一个额外的目标函数来鼓励他们有与x更高的相互信息。
最终code的独立的输入对应$$x$$一些特定的有意义的属性， 比如一个脸部图像的光照方向。

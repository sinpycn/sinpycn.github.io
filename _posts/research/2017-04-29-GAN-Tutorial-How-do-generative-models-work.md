...
给定一个数据集包含$$m$$个训练样本$$\mathbf{x}^{(i)}$$， 然后使用似然（likelihood）表示模型在训练数据集上的概率， 也就是: $$\prod_{i=1}^{m}p_{model}(x^{(i)};\theta)$$.
...
在实际应用中， 我们无法获取$$p_{data}$$本身， 只能使用包含有$$m$$个样本的训练数据集。 我们通过考虑这$$m$$个样本定义了一个经验性分布 $$\hat{p}_{data}$$来近似$$p_{data}$$. 最小化$$\hat{p}_{data}$$与$$p_{data}$$的KL散度等价于在训练数据集上最大化对数似然(log-likelihood).
...
正如我们前面提到过的，很多模型经常使用最大似然以外的原理， 为了减少比较的复杂度，我们可以评价他们的最大似然变量.
根据这种思路， 我们制作了图9的分类.
树中每一个叶子节点都有各自优点和缺点.
GAN被设计来避免很多的早已存在的缺点， 当然GAN同时也有一些新的缺点.

...
通过训练判别器， 我们可以获得一个对每一个x一个比值的估计。 

![Equation 9](/images/201705/03/eq09.jpg)

估计这个比值让我们计算一个范围很大的方差和导数。 这是让GAN区别于变分autoencoder和玻尔兹曼机的主要的优化技术。
其他的深度生成模型基于下边界或者马尔可夫链来优化； GAN优化基于使用监督学习来估计两个密度的比值。 GAN优化经常遭受失败监督学习失败， 过拟合和训练不足。 
在实际应用中， 完美的优化和足够的数据可以克服此问题。 其他的模型使用其他的优化也有其他的失败问题。

因为GAN可以很自然的分析使用游戏理论工具， 我们称GAN对抗。 但是我们也可以认为他们是合作的， 在这种情况下 判别器估计密度的比例， 并且自由的分析与生成器分享信息。 从这一点上看， 判别器更像一个老师指导生成器如何提高来超过对手。 当然，合作的观点没有带来对数学上开发的任何改变。

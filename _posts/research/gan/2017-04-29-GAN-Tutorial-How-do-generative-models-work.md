...
FVBNs的主要缺点是每一次计算只能生成一个条目（图像中，可以认为是一个点， WaveNet指一个frame）： 首先 $$x_1$$, 然后 $$x_2$$， 所以一个样本的生成复杂度为 $$O(n)$$。 最新的FVBN，比如WaveNet， 每一个$$x_i$$的分布都使用深度神经网络来计算， 所以每一个$$n$$步生成都意味着很大的计算量。
...
如果密度$$p_z$$是和$$g^{-1}$$的雅可比行列式都是可计算的，那么密度$$p_x$$也是可计算的。 
换句话说， 一个简单的关于$$z$$的分布与一个通过复杂的方式对空间进行变形（warp space）的$$g$$相结合，可以产生一个关于$$x$$的复杂分布。 
如果$$g$$经过仔细的设计， 那么密度也是可计算的。
使用非线性$$g$$函数的模型可以追溯到至少Deco and Brauer (1995)。 最近的工作是Real NVP(Dinh et al., 2016).
图10展示了使用Real NVP产生的ImageNet样本。
非线性ICA模型的主要缺点是对函数$$g$$的选择有很多的限制。 特别是因为有可逆的需求，所以隐变量$$z$$必须和$$x$$有相同的维数。
...
学习算法通过最大化$$\mathcal{L}$$至少可以保证得到一个与$$\zeta$$的同样高的对数似然值。
对于很多的模型来说， 即便是其对数似然不可计算，我们也可以设计一个易处理的$$\zeta$$。
当前，最流行的深度生成模型的变分方法是变分autoencoder(Kingma, 2013; Rezende et al., 2014) 或者被称为VAE（Variational autoencoder）。
VAE是当前最流行的三种生成模型之一。 主要的缺点是， 当使用很弱的一个近似的后验分布或者先验分布时[备注2]， 即便是有大量的数据并进行了完美的优化， $$\zeta$$与真实似然的差距会导致$$p_{model}$$无法表达真实的数据$$p_{data}$$。
...
`
备注2： 经验上来说， VAE使用很灵活的先验，或者很灵活的近似后验来更加靠近我们的对数似然$$\zeta$$ （Kingma  et al 2016; Chen et al., 2016b）。
...
有一些模型不需要定义显式的密度函数就可以被训练。 这些模型提供了一种与$$p_{mode}$$间接的交互的方法， 通常是使用它们（也就是$$p_{mode}$$）的采样数据。 图9中的右边的分支就是这种情况。

有些隐式模型基于根据$$p_{mode}$$绘制样本的方法， 这种模型定义了一个马尔可夫链转换操作， 此种方法需要运行很多次来获得一个样本。 
最主要的例子是生成随机网络（Bengio et al., 2014）。 正如2.3.2章提到的， 马尔可夫链无法处理高维空间问题，并且对于生成模型来说造成了计算量的提高。 GAN被设计为避免此问题。

最后， 图9最右边的分支是隐式模型， 此种模型通过一个步就可以生成样本。 在GAN提出的时候， GAN是属于此类的唯一方法， **但是从那以后， GAN被加到了其他的基于Kernelized momentmatching方法 （Li et al., 2015; Dziugaite et al., 2015）**。
